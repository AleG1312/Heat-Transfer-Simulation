...
This program provides a way to simulate heat transfer in a plate. In a real scenario, a scientist will apply constant heat in the plate borders and track the behaviour until equilibrium is reached. This state is achieved at the moment where each point of the plate does not change its temperature with time (or change is under a stablished margin). For simulation purposes, the plate will be considered as a matrix of `R` rows and `C` columns, with square cells of size `h`. 

[[matrix-heat-simulation]]
.Representation of the plate in the simulation. footnote:[Image adapted from https://jeisson.ecci.ucr.ac.cr/concurrente/2025b/tareas/#problem_statement[Problem Statement].]
...
IMPORTANT: If a cell temperature changes is lower than the thermal sensitivity, then the cell is at equilibrium. The plate equilibrium is when every cell reaches equilibrium.
...
To use this simulation, first be sure to have make and gcc installed. Otherwise, do it with the following commands:
...
In the repository, a makefile is provided to facilitate with the compilation and execution of the proyect. It can be used in the following way:
...
# To run the proyect with required CLI arguments
